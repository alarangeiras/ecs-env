...
async function getProperties(serviceName, region) {
	...
	const taskDefinition = await ecs
		.listTaskDefinitions({ familyPrefix: serviceName, sort: 'DESC' })
		.promise();
	...
	const taskDefinitionDescription = await ecs
		.describeTaskDefinition({ taskDefinition: firstTaskDefinitionArn })
		.promise();
